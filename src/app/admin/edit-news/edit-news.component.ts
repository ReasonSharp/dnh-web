  ...
  async ngOnInit() {
    try {
      this.posts = await firstValueFrom(
        this.http.get<PostItem[]>('/api/get_posts.php?type=news', { withCredentials: true })
      );
    } finally {
      this.loading = false;
    }
    ...
    try {
      const res = await firstValueFrom(
        this.http.post<{ success: boolean; message?: string }>(
          '/api/update_post.php',
          { type: 'news', id: this.selectedId, blurb: this.blurb, imageURL: this.imageUrl, body: this.body || null },
